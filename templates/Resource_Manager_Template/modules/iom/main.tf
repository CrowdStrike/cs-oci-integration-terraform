...
# Data Sources
# This will see if a the Default Identity Domain exists in the tenancy, to determine if tenancy uses Identity Domains or not
# (the resources that get deployed change depending on if a tenancy is using Identity Domains and data from the default domain is needed to configure some of the resources).
...
#  Checks that the tenancy's home region matches the value that was provided in the first step of the Falcon Cloud Security registration wizard
  lifecycle {
    precondition {
      ...
      error_message = "This tenancy has been configured in Falcon Cloud Security with a home region of ${var.expected_home_region}. It appears that the actual home region is ${data.oci_identity_region_subscriptions.homeregion.region_subscriptions[0].region_name}..."
    }
  }
}
...
resource "oci_identity_group" "fcs_inventory_group" {
  compartment_id = var.tenancy_ocid
  name          = var.group_name
  description   = "DO NOT TOUCH. Group for CrowdStrike Falcon Cloud Security (FCS) service account."
}
...
resource "oci_identity_policy" "fcs_inventory_policy_without_domains" {
  ...
  description    = "DO NOT TOUCH. This policy allows CrowdStrike Falcon Cloud Security to create an inventory"
  compartment_id = var.tenancy_ocid
  statements     = [
